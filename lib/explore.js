...
const parseExploreResults = (page, url) => {
    ...
    if (results.cider) {
        ...
        const headerMatch = page.match(/<strong>(\d+)<\/strong>\s+stamina.*<strong>(\d+)<\/strong>\s+items\s+found/)
        if (!headerMatch) {
            throw "Unable to parse stamina from icder results"
        }
        results.stamina = parseInt(headerMatch[1], 10)
        // We don't need it per se but useful for verification of the per-item parsing
        results.totalItems = parseInt(headerMatch[2], 10)
...
const visitExploreResults = async (state, page, url) => {
    ...
    for (const item of results.items) {
        ...
        if (item.overflow) {
            state.player.inventory[itemName] = state.player.maxInventory
        } else {
            state.player.inventory[itemName] += item.quantity
        }
