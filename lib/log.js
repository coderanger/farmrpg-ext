class ActionLog {
    ...
    async npcMail(results) {
        return await this.log({type: "npc_mail", results})
    }
}
...
const clickLog = async (state, eventType, eventArg, msg) => {
    if (msg.shift) {
        await downloadFile(await state.db.getAll("items"), "items.json")
        await downloadFile(await state.db.getAll("locations"), "locations.json")
        await downloadFile(await state.db.getAll("pets"), "pets.json")
        ...
        await downloadFile(await state.db.getAll("borgens"), "borgens.json")
        await downloadFile(await state.db.getAll("exchangeCenter"), "exchangeCenter.json")
        // Filter quests into two files.
        const quests = await state.db.getAll("quests")
        const normalQuests = []
        const personalQuests = []
        for (const quest of quests) {
            if (quest.isPersonal) {
                personalQuests.push(quest)
            } else {
                normalQuests.push(quest)
            }
        }
        await downloadFile(normalQuests, "quests.json")
        await downloadFile(personalQuests, "personalQuests.json")
    } else if (msg.meta) {
