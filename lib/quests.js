...
const parseQuestText = (text) => {
    ...
    match = text.match(/Requires:.*Farming Level (\d+)/msi)
    if (match) {
        quest.requiresFarming = parseInt(match[1], 10)
    }
    match = text.match(/Requires:.*Crafting Level (\d+)/msi)
    if (match) {
        quest.requiresCrafting = parseInt(match[1], 10)
    }
    match = text.match(/Requires:.*Fishing Level (\d+)/msi)
    if (match) {
        quest.requiresFishing = parseInt(match[1], 10)
    }
    match = text.match(/Requires:.*Exploring Level (\d+)/msi)
    if (match) {
        quest.requiresExploring = parseInt(match[1], 10)
    }
    match = text.match(/Requires:.*Tower Level (\d+)/msi)
    if (match) {
