    ...
    async learn(data) {
        ...
        for (const key in allKeys) {
            if (JSON.stringify(newData[key]) !== JSON.stringify(existing[key])) {
                console.log(`QuestDB: Changed key ${key}`, newData[key], existing[key])
                change = true
                break
            }
        }
...
const parseQuestText = (text) => {
    ...
    match = text.match(/Requires:.*Farming Level (\d+)/i)
    if (match) {
        quest.requiresFarming = parseInt(match[1], 10)
    }
    match = text.match(/Requires:.*Crafting Level (\d+)/i)
    if (match) {
        quest.requiresCrafting = parseInt(match[1], 10)
    }
    match = text.match(/Requires:.*Fishing Level (\d+)/i)
    if (match) {
        quest.requiresFishing = parseInt(match[1], 10)
    }
    match = text.match(/Requires:.*Exploring Level (\d+)/i)
    if (match) {
        quest.requiresExploring = parseInt(match[1], 10)
    }
    match = text.match(/Requires:.*Tower Level (\d+)/i)
    if (match) {
        quest.requiresTower = parseInt(match[1], 10)
    }
