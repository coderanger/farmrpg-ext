export class RequestInterceptor {
    /**
     * Initialize a new RequestInterceptor.
     * @param {(number, string, number) => void} latencyCallback
     */
    constructor(latencyCallback) {
    ...
    async listener(details) {
        ...
        const latencyCallback = this.latencyCallback
        filter.onstop = () => {
            const now = Date.now()
            let page = ""
            ...
            // Log the request latency. Not await'd so as to not increase latency.
            latencyCallback(now, details.url, now - details.timeStamp)
